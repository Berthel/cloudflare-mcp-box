/**
 * OAuth handler for Box upstream authentication.
 * Implements OAuth 2.0 authorization code flow against Box.
 *
...
const BOX_USERINFO_URL = "https://api.box.com/2.0/users/me";

export type BoxOAuthProps = {
  email: string;
  name: string;
  userId: string;
  accessToken: string;
...
app.use("*", async (c, next) => {
  const required = ["BOX_CLIENT_ID", "BOX_CLIENT_SECRET", "COOKIE_ENCRYPTION_KEY"] as const;
...
app.get("/authorize", async (c) => {
  ...
  return renderApprovalDialog(c.req.raw, {
    ...
    server: {
      name: "Box MCP Server",
      description: "Connect your AI assistant to Box for file management, search, and collaboration.",
      logo: "https://cdn.worldvectorlogo.com/logos/box-2.svg",
    },
    setCookie,
    state: { oauthReqInfo },
  });
...
app.get("/callback", async (c) => {
  if (c.req.query("error")) {
    ...
    return c.text(`Box authorization failed: ${error} — ${desc}`, 400);
  }
  ...
  const tokenResponse = await fetch(BOX_TOKEN_URL, {
    ...
    body: new URLSearchParams({
      ...
      client_id: c.env.BOX_CLIENT_ID,
      client_secret: c.env.BOX_CLIENT_SECRET,
  ...
  if (!tokenResponse.ok) {
    ...
    return c.text(`Failed to exchange authorization code with Box: ${errorText}`, 500);
  }
  ...
  const userResponse = await fetch(BOX_USERINFO_URL, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!userResponse.ok) {
    return c.text("Failed to fetch user info from Box", 500);
  }
  ...
  if (!isEmailAllowed(userData.login, c.env)) {
    return renderAccessDenied(userData.login);
  }
...
function renderAccessDenied(email: string): Response {
  ...
  return new Response(
    ...
    a{display:inline-block;margin-top:1rem;padding:.75rem 1.5rem;background:#0061d5;color:#fff;border-radius:8px;text-decoration:none;font-weight:500}</style></head>
    <body><div class="card">
    <h1>Access Denied</h1>
