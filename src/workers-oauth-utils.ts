 ...
 * Adapted from Cloudflare's workers-oauth-provider template.
 * See: https://github.com/cloudflare/ai/tree/main/demos/remote-mcp-github-oauth
 *
 * This file is generic — no provider-specific code. Copy as-is into new projects.
 */
...
export function generateCSRFProtection(): { token: string; setCookie: string } {
  ...
  const setCookie = `${csrfCookieName}=${token}; HttpOnly; Secure; Path=/; SameSite=Lax; Max-Age=1800`;
...
export async function bindStateToSession(stateToken: string): Promise<{ setCookie: string }> {
  ...
  const setCookie = `${consentedStateCookieName}=${hashHex}; HttpOnly; Secure; Path=/; SameSite=Lax; Max-Age=1800`;
  return { setCookie };
}
...
<head>
  ...
  <style>
    :root { --primary: #4f46e5; --border: #e5e7eb; --text: #333; --bg: #fff; }
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, sans-serif; line-height: 1.6; color: var(--text); background: #f9fafb; margin: 0; padding: 0; }
    .container { max-width: 500px; margin: 3rem auto; padding: 1rem; }
    .card { background: var(--bg); border-radius: 12px; box-shadow: 0 8px 36px rgba(0,0,0,0.1); padding: 2rem; }
    .header { text-align: center; margin-bottom: 1.5rem; }
    .logo { width: 48px; height: 48px; margin-bottom: 0.5rem; border-radius: 8px; }
    h1 { font-size: 1.3rem; font-weight: 500; margin: 0; }
    .desc { color: #666; text-align: center; margin-bottom: 1.5rem; }
    .client-info { border: 1px solid var(--border); border-radius: 8px; padding: 1rem; margin-bottom: 1.5rem; }
    .client-name { font-weight: 600; font-size: 1.1rem; margin: 0 0 0.5rem; }
    .info { color: #666; font-size: 0.9rem; margin-bottom: 1.5rem; text-align: center; }
...
<body>
  <div class="container"><div class="card">
    ...
    <div class="client-info">
      ...
      <p style="margin:0;color:#666;font-size:0.9rem;">wants to connect to ${serverName}</p>
    </div>
    <p class="info">You will be redirected to authorize this application.</p>
